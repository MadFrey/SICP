...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for v in word_list:
        if v == typed_word:
            return v    
        num =   diff_function(typed_word,v,limit)
        if num<min and num<=limit:
            min = num
            word = v
    return word
...
def feline_fixes(typed, reference, limit):
    ...
    # BEGIN PROBLEM 6
    count = 0 
    def dfs(i,typed,refer,limit):
        nonlocal count
        if count>limit:
            return 
        if i <len(typed) and i<len(refer):
            if refer[i] != typed[i]:
                count+=1
        else:
            count += len(reference)- i
            return  
        dfs(i+1,typed,refer,limit)    

    if len(typed)>len(reference):
        count+=len(typed) - len(reference)   
    dfs(0,typed,reference,limit)         
    return count 
...
def time_per_word(words, times_per_player):
    ...
    m = {"words":words}
    t = []
    for a in times_per_player:
        start = 0
        end = 1
        c = []
        while end < len(a):
            c.append(a[end]- a[start]) 
            start+=1
            end+=1
        t.append(c)
    m["times"] = t    
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    ans = [[] for _  in range(len(player_indices))]
    times = match["times"]
    words = match["words"]
