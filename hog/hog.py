...
def oink_points(player_score, opponent_score):
    ...
    if opponent_score >=10:
        ge = opponent_score%10 
        opponent_score//=10
        shi = opponent_score%10
        if shi*2 - ge >1:
            return shi*2 -ge
        return 1
    else:
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls!=0:
       ans = roll_dice(num_rolls,dice)
        ...
        ans = oink_points(player_score,opponent_score)
    return ans
    # END PROBLEM 3
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 == score1 :
        return None,None 
    if  last_leader!= 0 and score0>score1  :
        return 0,"Player 0 takes the lead by "+str(score0-score1)
    if  last_leader!= 1 and  score1>score0 :
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max = 0 
    for i in range(1,11):
        func =  make_averaged(roll_dice,total_samples)
        avg = func(i,dice)
        if avg > max:
            index = i
            max = avg
...
def oink_points_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    num =  oink_points(score,opponent_score)
    if num>=threshold:
        return 0
    return num_rolls
    # END PROBLEM 10
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    roll =  oink_points_strategy(score,opponent_score,threshold,num_rolls)
